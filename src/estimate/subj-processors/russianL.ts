...
export class RussianL extends AbstractProcessor {
  ...
  analyze(): object {
    ...
    this.setK1();
    if (this.criteria.K1 !== 0 || this.webMode) {
      this.setK2();
      this.setK3();
      this.setK4();
    }
  ...
  setK3(): void {
    if (
      this.formattedEr['позиция'] > 0 &&
      this.formattedEr['п.позиция'] === 0
    ) {
      this.criteria.K3 = 1;
    } else {
      this.criteria.K3 = 0;
  ...
  setK7(): void {
    this.criteria.K7 = Math.max(
      0,
      Math.floor(3 - 0.5 * this.formattedEr['ошорф']) - this.shortTextFlag,
    );
  }

  setK8(): void {
    this.criteria.K8 = Math.max(
      0,
      Math.floor(3.5 - 0.5 * this.formattedEr['ошпункт'] - this.shortTextFlag),
    );
  }
